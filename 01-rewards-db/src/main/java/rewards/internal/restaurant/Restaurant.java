 ...
 *
 * <p>Restaurants calculate how much benefit may be awarded to an account for dining based on an
 * availability policy and a benefit percentage.
 */
@Entity
...
public class Restaurant {
     ...
     *
     * <ul>
     *   <li>{@link #getDbBenefitAvailabilityPolicy()}
     *   <li>{@link #setDbBenefitAvailabilityPolicy(String)}
     * </ul>
     *
     * It is marked transient to hide it from the default field mapping mechanism.
     */
    @Transient
    ...
    /**
     * Sets the policy that determines if a dining by an account at this restaurant is eligible for
     * benefit.
     *
    ...
    /**
     * Returns this restaurant's benefit availability policy code for storage in the underlying
     * column. This method is a database-specific accessor using the JPA 2 @Access annotation.
     */
    ...
    protected String getDbBenefitAvailabilityPolicy() {
        if (benefitAvailabilityPolicy == AlwaysAvailable.INSTANCE) {
            ...
            throw new IllegalArgumentException(
                    "No policy code for " + benefitAvailabilityPolicy.getClass());
        }
    }

    /**
     * Sets this restaurant's benefit availability policy from the code stored in the underlying
     * column. This method is a database-specific accessor using the JPA 2 @Access annotation.
     */
    @Access(AccessType.PROPERTY)
    @Column(name = "BENEFIT_AVAILABILITY_POLICY")
